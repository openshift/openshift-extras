...
configure_named()
{
  ...
  keyfile=/var/named/${apps_domain}.key

  if [ "x$bind_key" = x ]
  then
    # Generate the new key for the domain.
    pushd /var/named
    rm -f /var/named/K${apps_domain}*
    dnssec-keygen -a HMAC-MD5 -b 512 -n USER -r /dev/urandom ${apps_domain}
    bind_key="$(grep Key: K${apps_domain}*.private | cut -d ' ' -f 2)"
    popd
  fi
...
EOF
  ...
  nsdb=/var/named/dynamic/${apps_domain}.db
  cat <<EOF > $nsdb
\$ORIGIN .
\$TTL 30;
${apps_domain}		IN SOA	${named_hostname}. hostmaster.${domain}. (
				2011112904 ; serial
				60         ; refresh (1 minute)
				15         ; retry (15 seconds)
				...
				10         ; minimum (10 seconds)
			                  ...
			                  NS	${named_hostname}.
${named_hostname}.	    A	${named_ip_addr}
EOF
  ...
  cat <<EOF > /var/named/${apps_domain}.key
key ${apps_domain} {
  algorithm HMAC-MD5;
  secret "${bind_key}";
};
...
include "${apps_domain}.key";

zone "${apps_domain}" IN {
	type master;
	file "dynamic/${apps_domain}.db";
	allow-update { key ${apps_domain} ; } ;
};
EOF
...
BIND_PORT=53
BIND_KEYNAME="${apps_domain}"
BIND_KEYVALUE="${bind_key}"
BIND_ZONE="${apps_domain}"
EOF
...
set_dns_key()
{
  ...
  bind_key=$(${SSH_CMD} ${named_hostname} "grep Key: /var/named/K${apps_domain}*.private | cut -d ' ' -f 2")
}
